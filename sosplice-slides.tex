...
\draw (sm) ++(4,-1.0)
    ...
    node (to) [right] {tcp\_output()};
\draw[->]
    (node cs:name=ti,anchor=east) -| ++(1.4,.5) ++(0,.3)
    node (rw) {sorwakeup()} ++(0,.3) |-
    (node cs:name=sm,anchor=south);
\draw[->]
    (node cs:name=to,anchor=west)
    node (ta) [left] {tcp\_input()}
    (node cs:name=ta,anchor=west) ++(.3,.3) -- ++(0,1)  ++(0,.3)
    node (ww) {sowwakeup()} ++(0,.3) |-
    (node cs:name=sm,anchor=south);
